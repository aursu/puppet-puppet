...
This is the same change [1.0.0](#release-100) made to `puppet::config::webserver`, one layer down, and for the same reason: binding a wildcard puts a fleet's control plane on every interface a host has, including ones added after the fact. It matters more here than it looks, because **PuppetDB authenticates but does not authorise.** A client certificate is already required at the TLS layer - a certless caller is refused during the handshake, not with a 403 - but `bootstrap.cfg` scopes the authorization service to the metrics service, so `auth.conf` governs only `/metrics` and `/status`, and **not `/pdb/*`**. Its `"/"` deny-all at sort-order 999 never applies to the data paths. Facts, catalogues, reports and exported resources are therefore authorised by nothing beyond "this certificate chains to our CA" - which, wherever one CA serves a whole fleet, is every agent certificate that CA ever signed. Binding to loopback removes the listener instead of putting a check in front of it, and no authorisation rule available on this service does that.
